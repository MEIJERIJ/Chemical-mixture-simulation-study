...
ridge <- function(obs, n_boot = 200, qlow, qhigh) {
  ...
  #---------------------------------
  cv_fit <- cv.glmnet(X, Y, alpha = 0, penalty.factor = penalty_factors, nfolds = 5)
  plot(cv_fit)
  title("Ridge CV (MSE Path)", line = 3)
  
  
  ### Fit final model with lambda.1se (more parsimonious)
  #------------------------------------------------------
  lasso_fit <- glmnet(X, Y, alpha = 0, penalty.factor = penalty_factors, 
  ...
  for(i in 1:n_boot) {
    ...
    cv_b <- cv.glmnet(X_b, Y_b, alpha = 0, penalty.factor = penalty_factors, 
                      nfolds = 5)
    fit_b <- glmnet(X_b, Y_b, alpha = 0, penalty.factor = penalty_factors, 
