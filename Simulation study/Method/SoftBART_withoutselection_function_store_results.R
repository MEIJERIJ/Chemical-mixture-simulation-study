################################################################################
#
# Function to store all the results from SoftBART (non-sparse)
#
################################################################################
...
#-----------------------------------------------
run_softbart_withoutselection <- function(df, qlow , qhigh, sim_id = 1, psi, joint_eff) {
  ### Input:
  # - df:         input dataset
  ...
  fit <- softbart_regression(Y ~ ., 
                             data = df, 
                             test_data = df,
                             hypers = Hypers(df[-1],df$Y ,num_tree = 200),
  ...
  coef_softbart <- data.frame(
    ...
    Method = paste0("SoftBART (non-sparse)"),
    Size = nrow(df),
