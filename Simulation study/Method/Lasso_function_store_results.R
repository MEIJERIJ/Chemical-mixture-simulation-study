...
### Function to store Lasso results
#----------------------------------
run_Lasso <- function(obs, n_boot = 200, sim_id = 1, qlow, qhigh,  psi, joint_eff){
  ### Input:
  # - obs:        input dataset
  ...
  #-------------------------
  cv_fit <- cv.glmnet(X, Y, alpha = 1, penalty.factor = penalty_factors,
  ...
  for(i in 1:n_boot){
    ...
    # Fit lasso regression using cross-validated lambda
  ...
  coef_lasso <- data.frame(
    ...
    Method = paste0("Lasso regression"),
    Size = nrow(obs),
