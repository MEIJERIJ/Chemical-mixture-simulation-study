################################################################################
#
# Function to store all the results from BART
#
################################################################################
...
### Function to do run BART
#--------------------------
run_bart <- function(df, q_low , q_high, sim_id = 1, psi, joint_eff) {
  ### Input:
  # - df:         input dataset
  ...
  ### Fit a BART model
  #-------------------
  fit <- gbart(df[-1],df$Y, ntree = 200, nskip = 5000, ndpost = 5000, sparse = FALSE)
  ...
  coef_softbart <- data.frame(
    ...
    parameter = c("Joint" , names(fit$varcount.mean)[1:7]),
    Method = paste0("BART"),
    Size = nrow(df),
