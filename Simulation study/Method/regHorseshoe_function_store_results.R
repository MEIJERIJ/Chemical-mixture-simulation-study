################################################################################
#
# Store results for Bayesian Regularised Horseshoe regression
# 
################################################################################
...
#-----------------------------------------------
run_reghorseshoe <- function(df, sim_id, psi, joint_eff, q_low, q_high) {
  ### Input:
  # - df:         input dataset
  ...
  prior <- c(
    ...
    #--------------------------
    set_prior("horseshoe(df = 1 , par_ratio = 4/3)", class = "b", nlpar = "pfas"),
  ...
  fit <- brm(
    ...
    prior = prior,
    seed    = 123,
    chains  = 4, cores = as.integer(Sys.getenv("SLURM_CPUS_PER_TASK", unset = "4")), iter = 5000,
  ...
  coef_horseshoe <- data.frame(
    ...
    Method = paste0("Regularised horseshoe regression"),
