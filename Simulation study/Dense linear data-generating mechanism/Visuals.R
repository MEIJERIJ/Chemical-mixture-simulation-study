...
output <- output %>% mutate(CI_width = if_else(Method %in% c("Lasso regression", "Elastic Net regression"), NA_real_, CI_width),
         CI_coverage = if_else(Method %in% c("Lasso regression", "Elastic Net regression"), NA_real_, CI_coverage),
         Power = if_else(Method %in% c("Lasso regression", "Elastic Net regression"), NA_real_, Power))
...
power_summary %>%
  ...
  filter(Method != "Elastic Net regression" & Method != "Lasso regression") %>%
  ...
  labs(color = NULL, shape = NULL) +          # ← merges into one legend, no title
  theme(
    strip.background   = element_blank(),
    ...
    legend.position    = "right",             # ← right side
...
power_summary %>%
  ...
  labs(fill = 'Method:') + 
  theme(
    strip.background  = element_blank(),            # <- remove grey strip background
    panel.grid.minor  = element_blank(),
    panel.grid.major.y = element_blank(),
    plot.title = element_text(size = 16),
    axis.title.y = element_text(size = 14),
